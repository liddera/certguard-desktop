...
export const PowerShellService = {
  ...
  async cleanupOrphanCerts(): Promise<void> {
    const script = `Get-ChildItem Cert:\\CurrentUser\\My | Where-Object { $_.NotAfter -lt (Get-Date).AddMinutes(-30) -and $_.HasPrivateKey } | ForEach-Object { try { Remove-Item -Path $_.PSPath -Force -ErrorAction Stop; Write-Output "Removed: $($_.Thumbprint)" } catch { } }`;
    try {
